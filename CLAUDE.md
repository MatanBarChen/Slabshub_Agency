...
**Brand design line (added 2026-08-26, Matan's standing directive: "אני רוצה שתמיד נלך בקו העיצוב של המותג"):** EVERY visual output — in-chat widgets, reports, landing pages, post visuals, email layouts — uses the SlabsHub design system defined in `docs/widget/`. Render via `scripts/widget.py`; never hand-roll a one-off palette. The system is the **live site's** editorial identity: honeycomb-yellow masthead `#ffd931`, 2px carbon-black rules, serif display (Playfair Display / Frank Ruhl Libre for Hebrew), Lora for figures, Heebo for UI labels, radii 12/25/45. Full token table and usage in `docs/widget/README.md`.

Two hard rules that come with it:
1. **Tokens are read from the live site, never remembered.** The Shopify theme settings are misleading — the live pages override them with their own CSS custom properties. Re-read `https://slabshub.com/` and update `docs/widget/style.css` whenever the site's design changes.
2. **Currency and number runs need `<span class="ltr">`.** In an RTL context `₪15–30` reorders to `15₪–30`. Semantic colours (green/amber/red) stay for status, in the editorial ink tones defined in the stylesheet.
